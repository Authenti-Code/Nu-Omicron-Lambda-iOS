...
import WebKit
import SDWebImage
class MessageDetailsViewController: UIViewController,APIsDelegate,WKNavigationDelegate {
    var data = [String:Any]()
    var id: String = ""
...
//        scrollView.contentSize = CGSize(width: UIScreen.main.bounds.width, height: UIScreen.main.bounds.height + lblDescription.intrinsicContentSize.height - 200)
    ...
    func getData(){
        ...
        API.shared.getData(url: API_ENDPOINTS.GET_MESSAGE_DETAILS.rawValue, viewController: self, params: params) { (response) in
            print(response)
...
//            self.lblDescription.attributedText = (data1["details"] as? String)?.htmlToAttributedString
            ...
            if let image = data1["image"] as? String{
                let imageURL = URL(string: API_URLS.BASE_URL_IMAGES_MESSAGE.rawValue +  (data1["image"] as! String))!
