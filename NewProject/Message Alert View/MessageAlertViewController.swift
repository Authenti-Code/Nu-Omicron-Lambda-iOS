...
class MessageAlertViewController: UIViewController,APIsDelegate {
    var data = [[String: Any]]();
    var current_page = 1
    ...
    func getData(){
        ...
        API.shared.getData(url: API_ENDPOINTS.GET_MESSAGE.rawValue, viewController: self, params: params) { (response) in
            print(response)
...
extension MessageAlertViewController: UITableViewDelegate, UITableViewDataSource{
...
//                cell.lblDesc.attributedText = (self.data[indexPath.row]["details"] as! String).htmlToAttributedString
//            }
//        }
        
//      cell.lblDesc.datade
        cell.lblDesc.isUserInteractionEnabled = true;
        _ = (data[indexPath.row]["created_at"] as! String).components(separatedBy: " ")
        cell.lblTime.text = Utility.formatDateMessage(date: data[indexPath.row]["created_at"] as! String)
//        cell.lblTimings.text =
        let date = Utility.formatTimeNew(str: data[indexPath.row]["created_at"] as! String)
        print(date)
...
//        let image1 = Utility.formatTimeNew(str: data[indexPath.row]["image"] as! String)
//        let image: String = API_URLS.BASE_URL_IMAGES_NEWS.rawValue + image1
//        let imageURL = URL(string: image)!
//        cell.oImageView.sd_imageIndicator = SDWebImageActivityIndicator.gray
//        cell.oImageView.sd_setImage(with: imageURL)
